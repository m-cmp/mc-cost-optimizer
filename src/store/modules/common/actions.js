...
const checkSubmittedSurvey = (context, payload) => {
  const fullPayload = {
    ...CHECK_SURVEY_SUBMITTED,
    ...payload
  };
  callCheckSurveySubmitted(fullPayload)
    .then((res) => {
      context.commit('SET_SUBMITTED_SURVEY_STATUS', res.Data);
    })
};

const submitSurvey = (context, payload) => {
  const fullPayload = {
    ...SUBMIT_SURVEY_REQUEST_MODEL,
    ...payload
  };
  return callSubmitSurvey(fullPayload);
};
