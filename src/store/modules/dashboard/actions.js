...
const getBudgetData = (context, payload) => {
  context.commit('SET_IS_BUDGET_DATA_LOADING', true);
  fetchBudgetData(payload)
    .then((res) => {
      context.commit('SET_IS_BUDGET_DATA_LOADING', false);
      context.commit('SET_BUDGET_DATA', res);
    })
    .catch((err) => {
      context.commit('SET_IS_BUDGET_DATA_LOADING', false);
      context.commit('SET_BUDGET_DATA', DEFAULT_WIDGET_DATA.BUDGET_DATA);
      console.error(err);
    })
};

const getTotalSaving = (context, payload) => {
  context.commit('SET_IS_TOTAL_SAVING_LOADING', true);
  fetchTotalSaving(payload)
    .then((res) => {
      context.commit('SET_IS_TOTAL_SAVING_LOADING', false);
      context.commit('SET_TOTAL_SAVING', res);
    })
    .catch((err) => {
      context.commit('SET_IS_TOTAL_SAVING_LOADING', false);
      context.commit('SET_TOTAL_SAVING', DEFAULT_WIDGET_DATA.TOTAL_SAVING);
      console.error(err);
    })
};
