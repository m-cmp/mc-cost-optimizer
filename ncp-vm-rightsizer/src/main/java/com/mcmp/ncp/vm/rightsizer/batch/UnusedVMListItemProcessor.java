...
public class UnusedVMListItemProcessor implements ItemProcessor<UnusedDto, UnusedDto> {
    ...
    public UnusedDto process(UnusedDto item) throws Exception {
        ...
        UnusedDto stats;
        try {
            stats = unusedProcessMartMapper.select14DaysMetricStats(item.getInstanceNo());
        } catch (Exception e) {
            log.error("Failed to query 14-day stats for Instance: {} - {}", item.getInstanceNo(), e.getMessage());
            return null;
        }

        // 데이터 존재 체크
        if (stats == null) {
            log.debug("No metric data found for Instance: {}", item.getInstanceNo());
            return null;
        }

        // 14일치 데이터 체크
        if (stats.getDayCount() == null || stats.getDayCount() < 14) {
            log.debug("Not enough 14-day data for Instance: {} (days={})",
                item.getInstanceNo(), stats.getDayCount() != null ? stats.getDayCount() : 0);
            return null;
        }

        // 평균/최대값 null 체크
        if (stats.getAvgCpu14Days() == null || stats.getMaxCpu14Days() == null) {
            log.debug("Invalid metric stats for Instance: {} (avg={}, max={})",
