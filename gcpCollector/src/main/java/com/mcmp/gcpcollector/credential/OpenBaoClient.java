package com.mcmp.gcpcollector.credential;
 ...
 * - VAULT_ADDR / VAULT_TOKEN 이 비어있으면 비활성(빈 맵 반환).
 * - provider 단위로 1회 조회 후 캐시 (기동 시 1회성 조회 가정).
 */
@Slf4j
@Component
public class OpenBaoClient {
    ...
    /** VAULT_ADDR/VAULT_TOKEN 이 모두 설정되어 있는지 */
    public boolean isConfigured() {
        return address != null && !address.isBlank() && token != null && !token.isBlank();
    }

    /** secret/data/csp/{provider} 의 key→value 맵 반환 (실패/미설정 시 빈 맵) */
    ...
    private Map<String, String> fetch(String provider) {
        ...
        try {
            ...
            JsonNode data = (body != null) ? body.path("data").path("data") : null; // KV v2: data.data
