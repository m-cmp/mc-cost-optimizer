# LLM 입력 데이터 정의서 (CPU-only)

> 우리 스코어링 API(`POST /ml-rightsize`)가 LLM에 넘기는 JSON의 **필드 정의 + 해석 규칙**이다.
> LLM 프롬프트에 컨텍스트로 함께 넣어주면, LLM이 각 값의 의미를 알고 추천을 생성한다.
> **※ 현재 CPU 메트릭만 수집되어 분석도 CPU 기준이다 (메모리 없음).**

---

## 1. 이 데이터는 무엇인가

가상 컴퓨트(EC2 등) **한 인스턴스**의 최근 약 1달치 **CPU 사용률**을 분석한 **자원 적정성 점수**다.
...
| 필드 | 의미 |
|---|---|
| `instance` | 인스턴스 식별자 (csp_instanceid) |
| `interval_seconds` | 수집 간격(초) — 데이터에서 자동 추론 |
| `window_days` | 실제 분석에 쓰인 데이터 기간(일) |
| `coverage` | 데이터 충실도 0~1 (낮으면 신뢰도 낮음) |
| `action_signal` | **1차 신호**: `downsize` / `keep` / `upsize` / `insufficient_data` |
| `efficiency_score` | 0~100, CPU 활용 효율 (**높을수록** 잘 씀 = 낭비 적음). 낮으면 다운사이즈 후보 |
| `headroom_risk` | 0~100, 포화 위험 (**높을수록** 위험). 높으면 업사이즈 후보 |
| `confidence` | 0~1, 이 판정의 신뢰도 (데이터 충실도·안정성·기간 반영) |
| `binding_resource` | 항상 `CPU` (CPU-only) |
| `reasons` | 판정 근거 문장들 (자연어 설명에 그대로 활용 가능) |
| `metrics.cpu.p95` | CPU 95퍼센타일 — "거의 항상 이 아래" (지속 부하) |
| `metrics.cpu.p99` | CPU 99퍼센타일 — 드문 피크 |
| `metrics.cpu.mean` | CPU 평균 |
| `metrics.cpu.idle_ratio` | 한가한(저활용) 시간 비율 0~1 |
| `metrics.trend_per_day` | CPU 추세 %/day (※ **거친 선형 추정치, 참고용**) |
| `metrics.bound_type` | 항상 `cpu-only` |
...
| `daily_shape.profile_24h` | 24개(0~23시) 시간대별 CPU 곡선 |
...
- **메모리는 미수집** → CPU만으로 판단한다. 메모리 병목 워크로드는 못 잡는다(현재 한계).
- `trend_per_day`는 거친 추정치다. 단독으로 강한 근거로 쓰지 말 것.
- `confidence`가 낮으면(예: < 0.5, 데이터 기간이 짧을 때) 추천 강도를 낮추거나 데이터 더 모으라고 권할 것.
